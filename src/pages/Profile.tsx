...
const Profile = () => {
    ...
    useEffect(() => {
        ...
        Promise.all([
            supabase
                .from("profiles")
                .select("name,certificate_name,avatar_url")
                .eq("id", user.id)
                .maybeSingle(),
            supabase
                .from("user_progress")
                .select("total_xp,total_gems,current_streak,longest_streak")
                .eq("user_id", user.id)
                .maybeSingle(),
            supabase
            ...
            setName(prof?.name ?? "");
            setCertName(prof?.certificate_name ?? "");
    ...
    const save = async () => {
        ...
        await supabase
            .from("profiles")
            .update({ name, certificate_name: certName })
            .eq("id", user.id);
        setBusy(false);
                        ...
                        <div className="relative flex flex-col gap-8">
                                ...
                                <div className="flex items-center gap-5">
                                    ...
                                    <div>
                                        ...
                                        <p className="mt-2 text-muted-foreground max-w-md">
                                            Your personal ZverTs identity, learning rewards, and
                                            account settings.
                                        </p>
                                    </div>
                                </div>
                            ...
                            <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
                                {[
                                    ...
                                    <div
                                        key={item.label}
                                        className="rounded-2xl border border-border/60 bg-background/50 p-4 shadow-card"
                                    >
                    ...
                    <div className="rounded-3xl border border-border bg-gradient-card p-8 shadow-card space-y-6">
                        ...
                        <div className="space-y-5">
                            ...
                            <Button
                                onClick={save}
                                disabled={busy}
                                className="w-full bg-gradient-lime text-primary-foreground hover:opacity-90 shadow-glow"
                            >
                                {t("common.save")}
                            </Button>
                        </div>

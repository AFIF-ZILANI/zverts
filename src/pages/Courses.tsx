...
interface Course {
  ...
  thumbnail_url: string | null; is_public: boolean; is_system: boolean;
  user_id: string | null; module_count?: number; completed_count?: number;
}
...
const Courses = () => {
  ...
  const load = async () => {
    ...
    if (!courses?.length) { setMine([]); setLoading(false); return; }
    ...
    const [{ data: mods }, { data: prog }] = await Promise.all([
      ...
      supabase.from("module_progress").select("module_id,completed")
        .eq("user_id", user.id)
        .in("module_id", (await supabase.from("modules").select("id").in("course_id", ids)).data?.map(m => m.id) ?? []),
    ]);
    ...
    setMine(courses.map(c => ({
      ...c,
      module_count: modsByCourse.get(c.id)?.length ?? 0,
      completed_count: (modsByCourse.get(c.id) ?? []).filter(mid => completedSet.has(mid)).length,
    })));
    setLoading(false);
  };
      ...
      <section className="container py-8 md:py-12 max-w-6xl">
        ...
        <div className="flex flex-wrap items-end justify-between gap-4 mb-8">
          <div>
            ...
            <h1 className="font-display text-4xl md:text-5xl font-semibold tracking-tight mt-1">{t("courses.mine")}</h1>
          </div>
        ...
        {loading ? (
          ...
          <div className="rounded-2xl border border-dashed border-border bg-card/50 p-12 text-center space-y-3">
            ...
            <p className="text-sm text-muted-foreground">Paste a YouTube playlist URL above to import your first course.</p>
          </div>
        ) : (
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {mine.map(c => (
    ...
    <div className="group rounded-2xl border border-border bg-card overflow-hidden hover:border-primary/40 hover:shadow-elevated transition-all duration-200">
      <Link to={`/courses/${c.id}`} className="block">
        <div className="relative aspect-video bg-muted overflow-hidden">
          {c.thumbnail_url
            ? <img src={c.thumbnail_url} alt="" className="w-full h-full object-cover group-hover:scale-[1.03] transition-transform duration-300" loading="lazy" />
        ...
        <div className="p-3 space-y-2">
          ...
          <div className="flex items-center justify-between text-xs text-muted-foreground font-mono">
            <span>{c.module_count} lesson{c.module_count !== 1 ? "s" : ""}</span>
          ...
          {(c.module_count ?? 0) > 0 && (
            <div className="space-y-1">
              ...
              <p className="text-[10px] font-mono text-muted-foreground">{c.completed_count}/{c.module_count} completed</p>
            </div>
          )}
        </div>
      </Link>
      <div className="px-3 pb-3 flex justify-end">
        <Button size="sm" variant="ghost" className="h-7 w-7 p-0 text-destructive opacity-0 group-hover:opacity-100 transition-opacity" onClick={onDelete}>

...
import {
  ArrowLeft, Lock, CheckCircle2, PlayCircle, Pencil,
  Trash2, Award, Share2, GripVertical, BookOpen, Clock,
} from "lucide-react";
...
interface Course {
  id: string; title: string; description: string | null; user_id: string | null;
  is_public: boolean; is_system: boolean; thumbnail_url: string | null;
  author_name?: string | null; author_channel_url?: string | null;
}
interface Module {
  id: string; position: number; title: string;
  duration_seconds: number; youtube_video_id: string; thumbnail_url: string | null;
}
...
// ── Sortable row ────────────────────────────────────────────────────────────
const SortableRow = ({
  m, owned, state, percent, onRename, onDelete,
}: {
  m: Module; owned: boolean; state: string; percent: number;
  onRename: (id: string, t: string) => void; onDelete: (id: string) => void;
}) => {
  const { attributes, listeners, setNodeRef, transform, transition, isDragging } =
    useSortable({ id: m.id, disabled: !owned });
  ...
  const row = (
    <div
      ...
      className={cn(
        ...
        locked ? "border-border/50 bg-muted/30 opacity-70" : completed ? "border-primary/30 bg-card" : "border-border bg-card hover:border-primary/40",
      )}
    >
      {/* drag handle */}
      {owned && (
        <button {...attributes} {...listeners} className="cursor-grab active:cursor-grabbing text-muted-foreground shrink-0 touch-none">
          <GripVertical className="h-4 w-4" />
        </button>
      )}

      {/* position */}
      <span className="font-mono text-xs text-muted-foreground w-6 shrink-0 text-center">{String(m.position).padStart(2, "0")}</span>

      {/* thumbnail */}
      {m.thumbnail_url && (
        <img src={m.thumbnail_url} alt="" className="h-10 w-[72px] object-cover rounded shrink-0" loading="lazy" />
      )}
      ...
      <div className="flex-1 min-w-0">
        {editing ? (
          <div className="flex gap-2">
            <Input value={val} onChange={e => setVal(e.target.value)} className="h-7 text-sm" />
            <Button size="sm" className="h-7 px-2" onClick={() => { onRename(m.id, val); setEditing(false); }}>Save</Button>
            <Button size="sm" variant="ghost" className="h-7 px-2" onClick={() => setEditing(false)}>Cancel</Button>
          </div>
        ) : (
          <>
            <p className={cn("text-sm leading-tight truncate", locked && "text-muted-foreground")}>{m.title}</p>
      ...
      {owned && !editing && (
        <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
          <Button size="icon" variant="ghost" className="h-7 w-7" onClick={() => { setVal(m.title); setEditing(true); }}>
            <Pencil className="h-3 w-3" />
          </Button>
          <Button size="icon" variant="ghost" className="h-7 w-7 text-destructive" onClick={() => onDelete(m.id)}>
...
const CourseDetail = () => {
  ...
  const load = async () => {
    ...
    setModules(m ?? []);
    if (user && m?.length) {
      const { data: p } = await supabase.from("module_progress")
        .select("module_id,percent_watched,completed,mcq_passed")
        .eq("user_id", user.id).in("module_id", m.map((x: any) => x.id));
      setProgress(p ?? []);
    }
    setLoading(false);
    if (c && !(c as any).author_name) {
      supabase.functions.invoke("fetch-playlist-author", { body: { course_id: c.id } }).then(({ data }) => {
        if (data?.author_name)
          setCourse(prev => prev ? { ...prev, author_name: data.author_name, author_channel_url: data.author_channel_url } : prev);
      });
    }
  };
  ...
  if (!course) return <AppShell><div className="container py-20 text-muted-foreground">Course not found.</div></AppShell>;
  ...
  return (
    <AppShell>
      <SEO
        title={course.title}
        description={course.description?.slice(0, 155) || `${course.title} — ${modules.length} lessons on ZverTs.`}
        path={`/courses/${course.id}`} type="article"
        jsonLd={{ "@context": "https://schema.org", "@type": "Course", name: course.title }}
      />
      <section className="container py-6 md:py-10 max-w-5xl">

        {/* Back */}
        <Link to="/courses" className="inline-flex items-center gap-1.5 text-xs font-mono text-muted-foreground hover:text-foreground mb-6">
          ...
          <div className="flex flex-col sm:flex-row gap-0">
            ...
            {course.thumbnail_url && (
              <div className="sm:w-56 lg:w-64 shrink-0">
                <img src={course.thumbnail_url} alt="" className="w-full h-full object-cover aspect-video sm:aspect-auto sm:h-full" />
              </div>
            )}
            {/* Info */}
            <div className="flex-1 p-5 md:p-6 flex flex-col justify-between gap-4">
              <div>
                {editingTitle && owned ? (
                  <div className="flex gap-2">
                    <Input value={titleVal} onChange={e => setTitleVal(e.target.value)} className="text-xl font-display h-10" />
                  ...
                  >
                    {course.title}
                    {owned && <Pencil className="inline h-3.5 w-3.5 text-muted-foreground ml-2 cursor-pointer" />}
                  </h1>
                )}
                {course.author_name && (
                  <p className="text-sm text-muted-foreground mt-1">
                    ...
                    {course.author_channel_url
                      ? <a href={course.author_channel_url} target="_blank" rel="noopener noreferrer" className="text-foreground hover:text-primary">{course.author_name}</a>
                      : <span className="text-foreground">{course.author_name}</span>
                    }
                    {" · "}YouTube
                  </p>
                )}
              ...
              <div className="flex flex-wrap gap-4 text-sm">
                ...
                <span className="font-medium text-foreground">{completedCount}/{modules.length} done</span>
              </div>

              {/* Progress bar */}
              <div>
                <div className="h-1.5 rounded-full bg-muted overflow-hidden">
                  <div className="h-full bg-gradient-lime transition-all duration-700" style={{ width: `${progressPct}%` }} />
                </div>
              </div>
                ...
                {nextUp && (
                  <Link to={`/learn/${nextUp.m.id}`}>
                    <Button className="bg-gradient-lime text-primary-foreground shadow-glow">
                      {nextUp.state === "in_progress" ? "Continue" : "Start learning"}
                    </Button>
                  </Link>
                )}
                {allDone && (
                  <Button className="bg-gradient-lime text-primary-foreground shadow-glow" onClick={() => navigate(`/certificate/${course.id}`)}>
          ...
          <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={onDragEnd}>
            <SortableContext items={modules.map(m => m.id)} strategy={verticalListSortingStrategy}>
              <div className="space-y-1.5">
                {cards.map(c => (
                  <SortableRow
                    key={c.m.id} m={c.m} owned={owned}
                    state={c.state} percent={c.percent}
                    onRename={onRenameModule} onDelete={onDeleteModule}
                  />
                ))}
              </div>
            </SortableContext>

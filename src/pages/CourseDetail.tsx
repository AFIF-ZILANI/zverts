...
const SortableRow = ({
  ...
  return (
    <div
      ref={setNodeRef}
      style={style}
      className={`group rounded-xl border ${locked ? "border-border/60 opacity-70" : completed ? "border-primary/40" : "border-border"} bg-gradient-card p-4 shadow-card flex items-center gap-3`}
    >
      {owned && (
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="text-xs text-muted-foreground font-mono mt-0.5">
          {fmt(m.duration_seconds)} · {Math.round(percent)}%
        </div>
      </div>
      {completed ? (
      ...
      {owned && (
        <>
          ...
          <Button size="icon" variant="ghost" className="h-8 w-8 text-destructive" onClick={() => onDelete(m.id)}>
  ...
  const load = async () => {
    ...
    if (c && !(c as any).author_name) {
      supabase.functions.invoke("fetch-playlist-author", { body: { course_id: c.id } }).then(({ data }) => {
        if (data?.author_name)
          setCourse((prev) =>
            prev ? { ...prev, author_name: data.author_name, author_channel_url: data.author_channel_url } : prev,
          );
      });
    }
  };
  ...
  return (
    <AppShell>
      <SEO
        title={course.title}
        description={
          course.description?.slice(0, 155) || `${course.title} — a course on ZverTs with ${modules.length} lessons.`
        }
      ...
      <section className="container py-8 md:py-10 max-w-5xl">
        <Link
          to="/courses"
          className="text-sm text-muted-foreground hover:text-foreground inline-flex items-center gap-1 font-mono mb-6"
        >
          ...
          {editingTitle && owned ? (
            <div className="flex gap-2 flex-1">
              <Input
                ...
                className="text-3xl font-display h-14"
              />
        ...
        <p className="text-sm text-muted-foreground font-mono mb-3">
          {completedCount} / {modules.length} {t("dashboard.completed").toLowerCase()}
        </p>

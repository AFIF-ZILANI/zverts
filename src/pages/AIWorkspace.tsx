    ...
    <div className="h-screen flex flex-col bg-background">
      ...
      <header className="shrink-0 h-14 border-b border-border/60 bg-background/80 backdrop-blur-xl flex items-center justify-between px-3 md:px-4 gap-2">
        <div className="flex items-center gap-2 min-w-0">
          <Link to="/dashboard" className="inline-flex h-8 w-8 items-center justify-center rounded-lg hover:bg-muted/60 transition-colors shrink-0" aria-label="Back to dashboard">
      ...
      <div className="flex-1 min-h-0 flex flex-col md:hidden">
        <div className="flex-1 min-h-0">
          {mobileTab === "sources" && <SourcesPanel userId={user.id} active={source} onSelect={(s) => { setSource(s); setMobileTab("chat"); }} />}
          {mobileTab === "chat" && (
            <ChatPanel
              userId={user.id}
              source={source}
          ...
          {mobileTab === "panel" && <TranscriptPanel source={source} onAskAbout={onAskAbout} />}
        </div>

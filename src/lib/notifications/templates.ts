...
const pick = <T>(arr: T[], seed = Date.now()): T => arr[Math.floor(seed % arr.length)] ?? arr[0];
...
const T = {
    morning_push: [
        ...
        (c) => ({
            title: "১টা lesson = level up?",
            body: `Level ${(c.level ?? 1) + 1} খুব কাছেই 🔥`,
        }),
    ] as Tpl[],
    afternoon_push: [
    ...
    evening_push: [
        () => ({
            title: "সন্ধ্যা + focus = combo 🔥",
            body: "এই সময়টাই best — ১টা lesson শেষ করেন",
        }),
        (c) => ({
            title: "Next lesson unlock! 🔓",
            body: `${c.moduleTitle ?? "Module"} ready, ready আপনি?`,
        }),
    ...
    streak_risk: [
        (c) => ({
            ...
            body: `${c.streak ?? 0} দিনের consistency ভেঙে যাইতেছে — ৫ মিনিট দেন!`,
        }),
    ...
    comeback_7d: [() => ({ title: "অনেকদিন দেখা নাই 👀", body: "ZverTs miss করতেছে 😄" })] as Tpl[],
    comeback_14d: [
    ...
    weak_topic: [
        (c) => ({
            title: `${c.weakTopic ?? "এই topic"} e একটু ঝামেলা?`,
            body: "AI ready আছে 😎 ২ মিনিট practice দেন",
        }),
        () => ({ title: "এই topic বারবার miss হচ্ছে 👀", body: "একটু revise করি?" }),
    ] as Tpl[],
    ai_recommendation: [
        (c) => ({
            title: "AI detect করছে 📚",
            body: `${c.subject ?? "আপনার favorite subject"} এ next lesson ready`,
        }),
    ...
    unfinished_lesson: [
        (c) => ({
            title: "Lesson half-done 👀",
            body: `${c.moduleTitle ?? "Lesson"} শেষ করেন — ৫ মিনিটই enough`,
        }),
    ] as Tpl[],
    system_success: [
    ...
    payment: [() => ({ title: "Payment success ✅", body: "Premium ready boss" })] as Tpl[],
    subscription: [() => ({ title: "Premium শেষ হতে চলতেছে ⏳", body: "Miss কইরেন না" })] as Tpl[],
} as const;

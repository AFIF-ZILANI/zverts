...
import {
  LogOut, LayoutDashboard, BookOpen, User as UserIcon,
  Shield, Menu, X, Sparkles, Bot, TrendingUp, CreditCard, Settings,
} from "lucide-react";
...
export const AppShell = ({ children }: { children: ReactNode }) => {
  ...
  return (
    <div className={cn("min-h-screen flex flex-col",
      theme !== "dark" && "bg-[radial-gradient(ellipse_at_top,_hsl(75_70%_50%_/_0.06),_transparent_60%)]")}>
      ...
      <header className="sticky top-0 z-40 border-b border-border/60 bg-background/80 backdrop-blur-xl">
        <div className="container flex h-14 items-center gap-3">
          ...
          >
            <img src={zvertsLogo} alt="ZverTs" className="h-9 w-auto object-contain" loading="eager" decoding="async" />
          </Link>
              ...
              {navItems.map(n => (
                <NavLink
                  ...
                  className={({ isActive }) => cn(
                    ...
                    n.to === "/admin" && "text-amber-500 hover:text-amber-400",
                  )}
                >
                  {({ isActive }) => (
                    <>
          ...
          <div className="hidden md:flex items-center gap-1">
            ...
            {user ? (
              <>
                ...
                {/* Upgrade CTA */}
                <Link to="/buy">
                  <Button size="sm" className="bg-gradient-lime text-primary-foreground shadow-glow ml-1 gap-1.5">
          ...
          <div className="md:hidden fixed left-0 right-0 top-14 z-40 border-b border-border/60 bg-card/98 backdrop-blur-xl shadow-2xl animate-fade-in max-h-[calc(100dvh-3.5rem)] overflow-y-auto">
            ...
            {user && (
              <nav className="px-3 py-3 grid grid-cols-2 gap-1.5">
                {navItems.map(n => (
                  <NavLink
                    ...
                    className={({ isActive }) => cn(
                      ...
                      n.to === "/admin" && !({ isActive: false }).isActive && "text-amber-500",
                    )}
                  >
                ...
                <NavLink
                  ...
                  className={({ isActive }) => cn(
                    "flex items-center gap-2.5 px-3 py-3 rounded-xl text-sm font-medium transition-colors",
                    isActive ? "bg-primary/10 text-primary" : "bg-muted/40 text-foreground/80 hover:bg-muted hover:text-foreground",
                  )}
                >
                ...
                <Link to="/buy" onClick={() => setMobileOpen(false)}>
                  <div className="flex items-center gap-3 px-4 py-3 rounded-xl bg-gradient-lime text-primary-foreground shadow-glow">
                    ...
                    <div>
                      ...
                      <p className="text-xs opacity-80">Unlock credits &amp; AI Tutor</p>
                    </div>
                  </div>
                </Link>
              </div>
            ...
            <div className="px-3 pb-4 pt-1 border-t border-border/60 flex items-center justify-between gap-2">
              ...
              {user ? (
                ...
                <Button size="sm" onClick={() => { setMobileOpen(false); navigate("/auth"); }}>
                  {t("nav.signin")}
                </Button>
              )}
            </div>

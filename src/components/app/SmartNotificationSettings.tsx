        ...
        <div className="rounded-2xl border border-border bg-gradient-card p-6 shadow-card space-y-5 mt-4">
            <div>
                ...
                <p className="text-xs text-muted-foreground">
                    Bangla study-buddy alerts — real-time, behaviour-aware.
                </p>
            </div>
            ...
            <div className="border-t border-border/60 pt-4 space-y-4">
                {row(
                    "Study reminders",
                    "Morning, evening, night nudges + streak risk.",
                    "study_reminders",
                )}
                {row("Gamification", "XP, level up, streak milestones, badges.", "gamification")}
                {row(
                    "AI suggestions",
                    "Summaries, generated quizzes, weak-topic alerts.",
                    "ai_suggestions",
                )}
                {row(
                    "System alerts",
                    "Playlist conversion, payments, subscription.",
                    "system_alerts",
                )}
            ...
            <div className="border-t border-border/60 pt-4 space-y-3">
                <Label className="text-sm">
                    ...
                    <span className="font-semibold text-primary">{p.max_per_day}</span>
                </Label>
                <Slider
                    value={[p.max_per_day]}
                    min={1}
                    max={10}
                    step={1}
                    onValueChange={([v]) => set("max_per_day", v)}
                />
            </div>
            <div className="grid grid-cols-2 gap-3">
                <div>
                    ...
                    <input
                        ...
                        onChange={(e) =>
                            set("quiet_hours_start", Math.min(23, Math.max(0, +e.target.value)))
                        }
                ...
                <div>
                    ...
                    <input
                        ...
                        onChange={(e) =>
                            set("quiet_hours_end", Math.min(23, Math.max(0, +e.target.value)))
                        }
                        className="mt-1 w-full rounded-md border bg-background px-3 py-2 text-sm"
                    />
                </div>

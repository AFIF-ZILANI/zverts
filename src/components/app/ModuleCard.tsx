...
export const ModuleCard = ({ position, title, durationSeconds, state, percent = 0, id }: Props) => {
    ...
    const inner = (
        ...
        >
            <div
                className={cn(
                    "transition-all",
                    locked && "blur-[2px] grayscale opacity-40 pointer-events-none",
                )}
            >
                ...
                <h3 className="font-display text-lg leading-tight text-balance min-h-[3.5rem]">
                    {title}
                </h3>
    ...
    return locked ? (
        <div aria-label="Locked module">{inner}</div>
    ) : (
        <Link to={`/learn/${id}`}>{inner}</Link>
    );
};

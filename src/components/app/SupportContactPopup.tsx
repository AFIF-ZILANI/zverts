  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const { data: profile } = await supabase
        ...
        .maybeSingle();
      if (cancelled) return;
      setName(profile?.name || profile?.certificate_name || user.user_metadata?.full_name || "");
      ...
      <DialogContent className="sm:max-w-md max-h-[92vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4 pt-2">
          <div>
            ...
            <Input id="sc-name" value={name} onChange={(e) => setName(e.target.value)} required className="mt-1.5" />
          </div>
          ...
          <div className="flex flex-col-reverse sm:flex-row gap-2 pt-2">
            <Button type="button" variant="ghost" className="sm:flex-1" onClick={handleLater} disabled={busy}>
              Maybe Later
            </Button>
            <Button type="submit" className="sm:flex-1 bg-gradient-lime text-primary-foreground hover:opacity-95 shadow-glow" disabled={busy}>

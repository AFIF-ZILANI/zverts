    ...
    try {
        ...
        if (course.author_name) {
            return json({
                ...
                author_channel_url: course.author_channel_url,
            });
        }
        ...
        const res = await fetch(
            `https://www.googleapis.com/youtube/v3/playlists?part=snippet&id=${course.source_playlist_id}&key=${apiKey}`,
        );
        ...
        const author_channel_url = author_channel_id
            ? `https://www.youtube.com/channel/${author_channel_id}`
            : null;
        ...
        await admin
            .from("courses")
            .update({ author_name, author_channel_id, author_channel_url })
            .eq("id", course.id);
